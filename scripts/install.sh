...
# check for ~/._sysconfig
if [[ ! -d "$SYS_CONFIG" ]] || [[ ! -d "$SYS_CONFIG/.git" ]]; then
  if [[ -d "$SYS_CONFIG" ]]; then
    # shellcheck disable=SC2088 # tilde does not expand in quotes
    read -r -p "~/._sysconfig already exists but is not a git repo.  Back up and overwrite (y/n)? [y] " overwrite_select
  ...
  echo "Cloning into $SYS_CONFIG"
  git clone https://github.com/ahgraber/sysconfig.git
  mv ./sysconfig "$SYS_CONFIG"
...
if [[ -d "${HOME}/.config/direnv" ]]; then
  ...
  # if exists as file, back up
  [[ -f "${HOME}/.config/direnv/direnvrc" ]] && mv "${SYS_CONFIG}/dotfiles/direnvrc" "${HOME}/.config/direnv/direnvrc.$(date +%Y%m%d)"
  # make link
